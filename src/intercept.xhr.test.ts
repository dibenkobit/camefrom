...
const server = Bun.serve({
    port: 0,
    fetch: () => Response.json({ items: [{ contractor: { name: 'ТОО Астана' } }] })
});
...
globalThis.XMLHttpRequest = window.XMLHttpRequest as unknown as typeof XMLHttpRequest;
intercept();
...
test('a body read over XHR and parsed by the app is traceable', async () => {
    ...
    expect(findReads('ТОО Астана')).toEqual([{ responseId: 1, path: 'items[0].contractor.name' }]);
});
