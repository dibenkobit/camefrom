...
export function recordResponse(meta: RequestMeta, body: unknown): RecordedResponse {
 ...
 */
export function recordRead(responseId: number, path: string, value: unknown): void {
    const existing = readsByValue.get(value);
    if (existing) {
        const known = existing.some((read) => read.responseId === responseId && read.path === path);
