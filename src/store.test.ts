...
import { findReads, findResponse, recordRead, recordResponse, reset } from './store';

const meta = { method: 'GET', url: '/api/works', status: 200, startedAt: 0, durationMs: 1 };
...
describe('reads', () => {
    test('are found by the value that was read', () => {
        ...
        expect(findReads('ТОО Барыс')).toEqual([{ responseId: 1, path: 'items[0].name' }]);
    });
